...
proc extract_from_power_rpt {power_rpt} {
    ...
    set fp [open $power_rpt r]
    while { [gets $fp line] >= 0 } {
        if { [lindex $line 0] == "Total" && [lindex $line 1] == "Leakage" && [llength $line] == 5 } {
            set leakage_power [lindex $line 3]
            break
        }
    }
    close $fp
    ...
    set fp [open $power_rpt r]
    while { [gets $fp line] >= 0 } {
        if { [lindex $line 0] == "Total" && [lindex $line 1] == "Switching" && [llength $line] == 5 } {
    ...
    set fp [open $power_rpt r]
    while { [gets $fp line] >= 0 } {
        if { [lindex $line 0] == "Total" && [lindex $line 1] == "Internal" && [llength $line] == 5 } {
            set internal_power [lindex $line 3]
            break
        }
    }
    close $fp

    # Convert to float and compute dynamic power
    if { $switching_power ne "" && $internal_power ne "" } {
        set dynamic_power [expr {$switching_power + $internal_power}]
    } else {
        puts "❌ Failed to extract power values."
    }

    set ans [list $total_power $leakage_power $dynamic_power]
...
proc extract_report {stage} {
    ...
    set ans "$stage,[dbget top.fplan.coreBox_area],[lindex $rpt3 1],[lindex $rpt3 0],[lindex $rpt2 0], [lindex $rpt2 1], [lindex $rpt2 2], $rpt4,[lindex $rpt1 0],[lindex $rpt1 1],[lindex $rpt1 2],[lindex $rpt1 3]"
    return $ans
}
